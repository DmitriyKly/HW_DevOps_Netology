resource "local_file" "ansible_inventory" {
  ...
  content = templatefile("${path.module}/inventory.tftpl", {
    ...
    databases = [for instance in yandex_compute_instance.db_vm : {
      ip = instance.network_interface[0].nat_ip_address,
      fqdn = instance.hostname
    }]
    storage_server = [for instance in yandex_compute_instance.storage : {
      ip = instance.network_interface[0].nat_ip_address,
      fqdn = instance.hostname
    }]
  })
...
resource "null_resource" "web_hosts_provision" {
  ...
  depends_on = [yandex_compute_instance.web, yandex_compute_instance.db_vm, yandex_compute_instance.storage]
